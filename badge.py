...
def draw_badge():
    ...
    display.thickness(2)
    company_size = 1.0  # A sensible starting scale
    while True:
        company_length = display.measure_text(company, company_size)
        if company_length >= (TEXT_WIDTH - COMPANY_PADDING) and company_size >= 0.1:
            company_size -= 0.01
        else:
            display.text(company, COMPANY_PADDING, (COMPANY_HEIGHT // 2) + 1, company_size)
            break
...
# company = truncatestring(company, company_size, TEXT_WIDTH)
